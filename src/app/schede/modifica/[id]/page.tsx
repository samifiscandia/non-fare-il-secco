...
export default function ModificaScheda({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Carica prima gli esercizi disponibili
        const exercisesResponse = await fetch('/api/esercizi');
        if (exercisesResponse.ok) {
          const exercises = await exercisesResponse.json();
          setAvailableExercises(exercises);
        }

        // Poi carica la scheda
        const workoutResponse = await fetch(`/api/schede/${resolvedParams.id}`);
        if (workoutResponse.ok) {
          ...
          // Mappa gli esercizi mantenendo il riferimento all'ID
          const mappedExercises = workout.exercises.map(ex => ({
            exercise: ex.exercise._id, // Usa l'ID dell'esercizio
    ...
    setWorkoutExercises(prev => {
      ...
      updated[index] = {
        ...
        exerciseName: field === 'exercise' 
          ? availableExercises.find(e => e._id === value)?.name 
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6">Modifica Scheda</h1>
